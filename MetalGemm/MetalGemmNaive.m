...
- (void)_setThreadGroups {
    _threadGroupSize = MTLSizeMake(4, 8, 1);
    
    NSUInteger width  = _m % 8 ? (_m + 8) / 8 : _m / 8;
    NSUInteger height = _n % 8 ? (_n + 8) / 8 : _n / 8;
    _threadGroups = MTLSizeMake(
                                (width % _threadGroupSize.width) ?
                                (width + _threadGroupSize.width) / _threadGroupSize.width :
                                width / _threadGroupSize.width,
                                (height % _threadGroupSize.height) ?
                                (height + _threadGroupSize.height) / _threadGroupSize.height :
                                height / _threadGroupSize.height,
                                1
                                );
    
    [_encoder dispatchThreadgroups:_threadGroups threadsPerThreadgroup:_threadGroupSize];
}
